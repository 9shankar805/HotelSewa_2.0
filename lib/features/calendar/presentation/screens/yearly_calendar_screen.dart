...
class _YearlyCalendarScreenState extends State<YearlyCalendarScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final data = await CalendarService.getYearlyCalendar(
        token: token,
        year: _year,
      );
      if (mounted) {
        setState(() {
          // monthly_occupancy: { "1": 0.62, ... }
          final raw = data['monthly_occupancy'];
          if (raw is Map) {
            _occupancy = raw.map((k, v) => MapEntry(k.toString(), (v as num).toDouble()));
          }
          // blocked_dates: ["2025-3-15", ...]
          final blocked = data['blocked_dates'];
          if (blocked is List) {
            _blocked = Set<String>.from(blocked.map((e) => e.toString()));
          }
          _isLoading = false;
        });
      }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(icon: const Icon(Icons.today_rounded), onPressed: () { setState(() => _year = DateTime.now().year); _load(); }, tooltip: 'Go to current year'),
        ],
      ),
      body: _isLoading

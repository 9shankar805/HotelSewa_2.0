...
class OrdersService {
  ...
  static Future<List<Map<String, dynamic>>> getMyOrders() async {
    final response = await ApiService.get(ApiConfig.myOrdersEndpoint, token: _token);
  ...
  static Future<Map<String, dynamic>> cancelOrder(String orderId) async {
    final response = await ApiService.post(ApiConfig.buildPath(ApiConfig.orderCancelEndpoint, '$orderId/cancel'), token: _token);
    if (response['success'] == true) return response['data'] ?? {};
  ...
  static Future<Map<String, dynamic>> updateMenuItem(
    ...
    if (image != null) {
      final fields = data.map((k, v) => MapEntry(k, v.toString()));
      return ApiService.uploadFile(ApiConfig.buildPath(ApiConfig.ownerMenuDetailEndpoint, itemId), image, token: _token, fields: fields);
    }
    final response =
        await ApiService.post(ApiConfig.buildPath(ApiConfig.ownerMenuDetailEndpoint, itemId), token: _token, data: data);
    if (response['success'] == true) return response['data'] ?? {};
  ...
  static Future<void> deleteMenuItem(String itemId) async {
    final response = await ApiService.delete(ApiConfig.buildPath(ApiConfig.ownerMenuDetailEndpoint, itemId), token: _token);
    if (response['success'] != true) {

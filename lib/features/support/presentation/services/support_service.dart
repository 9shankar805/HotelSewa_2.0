...
class SupportService {
  ...
  Future<List<Map<String, dynamic>>> getSupportTickets() async {
    final response = await ApiService.get(ApiConfig.supportTicketsEndpoint, token: _token);
  ...
  Future<Map<String, dynamic>> createSupportTicket(Map<String, dynamic> ticketData) async {
    final response = await ApiService.post(ApiConfig.supportTicketsEndpoint, token: _token, data: ticketData);
    if (response['success'] == true) {
  ...
  Future<Map<String, dynamic>> getTicketDetail(String ticketId) async {
    final response = await ApiService.get(ApiConfig.buildPath(ApiConfig.supportTicketDetailEndpoint, ticketId), token: _token);
    if (response['success'] == true) {
      return response['data'] ?? {};
    }
  ...
  Future<Map<String, dynamic>> startSupportChat(Map<String, dynamic> data) async {
    final response = await ApiService.post(ApiConfig.supportChatStartEndpoint, token: _token, data: data);
    if (response['success'] == true) {
      return response['data'] ?? {};
    }
  ...
  Future<Map<String, dynamic>> getSupportChat(String chatToken) async {
    final response = await ApiService.get(ApiConfig.buildPath(ApiConfig.supportChatGetEndpoint, chatToken), token: _token);
    if (response['success'] == true) {
      return response['data'] ?? {};
    }
  ...
  Future<void> endSupportChat(String chatToken) async {
    await ApiService.post(ApiConfig.buildPath(ApiConfig.supportChatEndEndpoint, '$chatToken/end'), token: _token);
  }
}

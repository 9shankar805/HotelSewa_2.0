...
class OffersService {
  ...
  static Future<Map<String, dynamic>> createOffer(Map<String, dynamic> data) async {
    final response = await ApiService.post(ApiConfig.ownerOffersEndpoint, token: _token, data: data);
  ...
  static Future<Map<String, dynamic>> updateOffer(String id, Map<String, dynamic> data) async {
    final response = await ApiService.post(
      ApiConfig.buildPath(ApiConfig.ownerOffersDetailEndpoint, id),
      token: _token,
      data: data,
    );
    if (response['success'] == true) return response['data'] ?? {};

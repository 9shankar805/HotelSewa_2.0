...
class ProfileService {
  ...
  Future<Map<String, dynamic>> getUserProfile() async {
    final response = await ApiService.get(ApiConfig.getOwnerEndpoint, token: _token);
  ...
  Future<Map<String, dynamic>> updateProfile(Map<String, dynamic> profileData) async {
    final response = await ApiService.post(ApiConfig.updateProfileEndpoint, token: _token, data: profileData);
    if (response['success'] == true) {
      return response['data'] ?? {};
    }
  ...
  Future<void> deleteAccount() async {
    final response = await ApiService.delete(ApiConfig.deleteUserEndpoint, token: _token);
    if (response['success'] != true) {
  ...
  Future<Map<String, dynamic>> getVerificationFields() async {
    final response = await ApiService.get(ApiConfig.verificationFieldsEndpoint, token: _token);
    if (response['success'] == true) {
      return response['data'] ?? {};
    }
  ...
  Future<Map<String, dynamic>> sendVerificationRequest(Map<String, dynamic> data) async {
    final response = await ApiService.post(ApiConfig.sendVerificationRequestEndpoint, token: _token, data: data);
    if (response['success'] == true) {
      return response['data'] ?? {};
    }
  ...
  Future<Map<String, dynamic>> getVerificationRequest() async {
    final response = await ApiService.get(ApiConfig.verificationRequestEndpoint, token: _token);
    if (response['success'] == true) {
      return response['data'] ?? {};
    }
  ...
  Future<void> logout() async {
    try {
      await ApiService.post(ApiConfig.logoutEndpoint, token: _token);
    } catch (_) {}
    _token = null;

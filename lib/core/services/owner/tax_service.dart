...
class TaxService {
  ...
  static Future<Map<String, dynamic>> saveTax(Map<String, dynamic> data) async {
    final response = await ApiService.post(ApiConfig.taxesEndpoint, token: _token, data: data);
  ...
  static Future<void> deleteTax(String id) async {
    final response = await ApiService.delete(ApiConfig.buildPath(ApiConfig.taxesEndpoint, id), token: _token);
    if (response['success'] != true) {
  ...
  static Future<Map<String, dynamic>> getTaxReport({
    ...
    final response = await ApiService.get(ApiConfig.taxesReportEndpoint, token: _token, queryParams: params);
    if (response['success'] == true) return response['data'] ?? {};
  ...
  static Future<Map<String, dynamic>> exportTaxReport({
    ...
    if (from != null) params['from'] = from;
    if (to != null) params['to'] = to;
    final response = await ApiService.get(ApiConfig.taxesReportExportEndpoint, token: _token, queryParams: params);
    if (response['success'] == true) return response['data'] ?? {};
